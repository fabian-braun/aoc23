...
fn main() {
    ...
    let (part_1, part_2): (Vec<i64>, Vec<i64>) = file_context
        .lines()
        .map(|line| {
            let seq = line
                .split(' ')
            ...
            let part_1_result = finals.iter().rev().fold(0, |acc, x| acc + x);
            let part_2_result = firsts.iter().rev().fold(0, |acc, x| x - acc);
            (part_1_result, part_2_result)
        })
        .unzip();

    println!(
        "Part I solution: {}, Part II solution {}",
        part_1.iter().sum::<i64>(),
        part_2.iter().sum::<i64>()
    );
}
